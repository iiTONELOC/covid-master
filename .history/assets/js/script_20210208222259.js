...
//BEGINING OF DRINK FUNCTION (REFATOR) EASIER TO READ SEPARATING FRUNCT FOR FETCH/API CALL (MH)(AT)
...
var reFetch = function (event) {
   ...
   var reFetch = function (event) {   showButton();   event.preventDefault()   console.log(event.target)   if (event.target.getAttribute("data-movie") === null      || event.target.getAttribute("data-meal") === null      || event.target.getAttribute("data-drink") === null) {      return;   }   movieHistoryID = event.target.getAttribute('data-movie')   mealHistoryID = event.target.getAttribute("data-meal")   drinkHistoryID = event.target.getAttribute("data-drink")   //call the api   movieHistory();   mealHistory();   drinkHistory();
}
   //call the api
   movieHistory();
   mealHistory();
